...
if __name__ == "__main__":
    ...
    print("Feature importance using calibrated models")
    ...
    if snakemake.params[5] == False:
        mod_idx = 0
    else:
        mod_idx = 1
    
    all_models = fitted_mod.steps[mod_idx][1].calibrated_classifiers_
    importances = np.zeros((10, fitted_mod.steps[mod_idx][1].n_features_in_))
    for i in range(0, len(all_models)):
        importances[i,:] = all_models[i].base_estimator.feature_importances_
    labels = feat.columns
