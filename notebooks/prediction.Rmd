...
read_csv(imp_files[5]) %>% select(-1) %>% pivot_longer(everything(), names_to = "pathway") %>% 
    left_join(annotation) %>% 
    mutate(full_name = paste(pathway, annotation, sep = "\n")) %>% 
    ggplot(aes(x = full_name, y = value)) + geom_jitter() + 
    geom_boxplot() + coord_flip() + scale_x_discrete(expand = c(0.01,0.01))
