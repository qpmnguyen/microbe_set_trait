...
```{r}
test <- weissman %>% select(species_tax_id, pathways) 
test


t_unit <- test$pathways[[1]]

t_unit %>% pivot_longer(everything()) %>% 
    mutate(func_name = map_chr(name, str_split(name, pattern = ".."))) %>% print(n = Inf)
```
