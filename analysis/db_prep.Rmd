    ...
    if (nrow(string) == 0){
        ...
        string <- string %>% 
            ...
            mutate(name = if_else(str_detect(name, "synthesis_"), 
                                  true = paste(rev(str_split(name, pattern = "_", n = 2)[[1]]), collapse = "_"),
                                  false = name)) %>% 
            ungroup() %>%
...
proc_substrate <- function(unit){
    ...
    string %>% mutate(name = str_replace(name, "^(.*)\\.\\.", "")) %>%
        print(n = Inf)
    
    
}

test <- weissman %>% select(species_tax_id, substrate)
unit <- test$substrate[[1]]



test %>% mutate(pathways = map_chr(pathways, proc_pathways))
